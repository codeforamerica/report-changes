class TellUsMoreAboutTheLostJobForm < Form
  ...
  def save
    ...
    report.current_change.update(attributes.except(
                                   :last_day_year,
        :last_day_month,
        :last_day_day,
        :last_paycheck_year,
  ...
  def self.existing_attributes(report)
    ...
    %i[year month day].each do |sym|
      attributes["last_day_#{sym}"] = report.current_change.last_day.try(sym)
      attributes["last_paycheck_#{sym}"] = report.current_change.last_paycheck.try(sym)
    end
