class TellUsMoreAboutTheLostJobForm < Form
  set_attributes_for :report, :last_day_year, :last_day_month, :last_day_day,
  ...
  def self.existing_attributes(report)
    ...
    %i[year month day].each do |sym|
      attributes["last_day_#{sym}"] = report.last_day.try(sym)
      attributes["last_paycheck_#{sym}"] = report.last_paycheck.try(sym)
    end
