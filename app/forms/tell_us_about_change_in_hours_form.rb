class TellUsAboutChangeInHoursForm < Form
  ...
  def save
    ...
    report.reported_change.update(attributes.except(:change_date_day, :change_date_month, :change_date_year))
  end

  def self.existing_attributes(report)
    attributes = report.reported_change.attributes
    %i[year month day].each do |sym|
      attributes["change_date_#{sym}"] = report.reported_change.change_date.try(sym)
    end
